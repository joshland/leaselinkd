...
- **Configuration** – All runtime parameters live in `/etc/leaselinkd/config.json`. OPNsense API secrets (`api_key`, `api_secret`) are kept separate in `/etc/leaselinkd/secrets.json`.
...
- **Reconciliation** – At startup and at the configured interval, the manager compares its SQLite desired state with owned OPNsense overrides and calls the OPNsense CRUD API to remove stale records and reassert desired records.
